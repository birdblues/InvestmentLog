CREATE OR REPLACE VIEW public.view_portfolio_factor_risk_contrib_multi_z AS
WITH latest AS (
    SELECT max(portfolio_date) AS portfolio_date
    FROM public.view_portfolio_factor_exposure_multi_z
),
exposures AS (
    SELECT *
    FROM public.view_portfolio_factor_exposure_multi_z
...
dates_raw AS (
    ...
    FROM public.view_factor_returns_zscore fr
    JOIN exposures e
...
cov AS (
    SELECT
        ...
        sum((r1.ret_z - m1.mean_ret) * (r2.ret_z - m2.mean_ret))
